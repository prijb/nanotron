// Electron candidate observables
//
//
//
//

#ifndef nanotron_DataFormats_ElectronCandidateFeatures_h
#define nanotron_DataFormats_ElectronCandidateFeatures_h
    ...
    bool operator<(const ElectronCandidateFeatures& other) const {
        if (IP2dSig > 0 and other.IP2dSig > 0) {
            if (std::fabs(IP2dSig - other.IP2dSig) > std::numeric_limits<float>::epsilon()) {
                // sort decreasing
                return std::fabs(IP2dSig) > std::fabs(other.IP2dSig);
            }
        }
        // sort decreasing
